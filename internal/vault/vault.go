...
func validate(data Data) error {
	...
	for name, key := range data.Keystores {
		...
		if strings.TrimSpace(key.KeyAlias) == "" || len(key.KeyAlias) > 255 {
			return errors.New("invalid key alias")
		}
		if key.StorePass == "" || len(key.StorePass) > maxPasswordSize ||
			key.KeyPass == "" || len(key.KeyPass) > maxPasswordSize {
			return errors.New("invalid keystore password")
		}
	}
	return nil
}
